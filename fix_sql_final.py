...
# -*- coding: utf-8 -*-
"""
Correction finale complète de tous les fichiers SQL
"""
...
def extract_values_correctly(values_text):
    """Extrait les valeurs SQL en gérant correctement les chaînes multi-lignes"""
    values = []
    current = ""
    in_string = False
    string_char = None
    ...
    while i < len(values_text):
        ...
        if char in ("'", '"') and (i == 0 or values_text[i-1] not in ['\\', "'"]):
            if not in_string:
                # Début de chaîne - parser toute la chaîne
...
def fix_courses_insert(content):
    ...
    def fix_match(match):
        ...
        # Identifier quelles valeurs on a déjà
        field_to_value = {}
