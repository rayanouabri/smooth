...
def fix_courses_insert(content):
    ...
    def fix_match(match):
        ...
        # Vérifier si rating manque (pattern: TRUE suivi directement de reviews_count)
        # Pattern à chercher : TRUE,\s*\d+,\s*\d+ (sans rating entre TRUE et reviews_count)
        if re.search(r'TRUE\s*,\s*\d+\s*,\s*\d+\s*\)', values_text):
            # Il manque rating. Insérer 4.5 (ou 4.8 selon le pattern du fichier source)
            # Chercher le pattern: TRUE suivi d'un nombre (reviews_count) puis un autre nombre (enrolled_count)
            fixed_values = re.sub(
                r'TRUE\s*,\s*(\d+)\s*,\s*(\d+)',
                r'TRUE,\n  4.8,\n  \1,\n  \2',
                values_text
